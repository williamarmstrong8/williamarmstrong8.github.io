...
for (let i = 0; i < selectItems.length; i++) {
  selectItems[i].addEventListener("click", function () {
    const selectedValue = this.innerText.toLowerCase().replace(" ", "-");
...
const showProjectDetails = function (projectId) {
  // Hide any open project details sections
  projectDetailsSections.forEach(section => section.classList.remove("active"));

  // Find and show the target project details section
  const targetSection = document.querySelector(`#${projectId}`);
  if (targetSection) {
    targetSection.classList.add("active");
    window.scrollTo(0, targetSection.offsetTop); // Scroll to the project details
  }
};
...
// Add click event to all close buttons in project details
